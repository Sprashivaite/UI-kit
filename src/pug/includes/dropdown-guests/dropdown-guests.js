...
(function ($) {
  const defaults = {
    ...
    setSelectionText (itemCount, totalItems) {
      const usePlural = totalItems !== 0 && this.textPlural.length > 0;
      const text = usePlural ? this.textPlural : this.selectionText;
      if (totalItems == 0) {totalItems = ''; $('.iqdropdown__clear .button__text_empty').hide();}
      else {$('.iqdropdown__clear .button__text_empty').show();}
  ...
  $.fn.iqDropdown = function (options) {
    this.each(function () {
      ...
      function addControls (id, $item) {
        ...
        $('.iqdropdown__clear').click(function(){
            totalItems = 0;
            itemCount[id] = 0;
